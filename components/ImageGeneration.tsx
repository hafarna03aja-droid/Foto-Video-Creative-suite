...
export const ImageGeneration: React.FC<ImageGenerationProps> = ({ onGenerateVideo, initialPrompt, onPromptConsumed, onCreationComplete, onEditImage }) => {
  ...
  const renderGenerateMode = () => (
    <>
      <p className="text-gray-400 mb-6">Buat gambar menakjubkan berkualitas tinggi dari perintah teks menggunakan Imagen 4.0. Pilih rasio aspek yang Anda inginkan.</p>
      <form onSubmit={handleGenerateSubmit} className="space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          <input
            ...
            className="flex-grow p-3 bg-gray-700 rounded-lg border border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
            disabled={isLoading}
          />
          <button
            type="submit"
  ...
  const renderBlendMode = () => (
    <>
      <p className="text-gray-400 mb-6">Padukan beberapa gambar dengan perintah teks untuk menciptakan hasil yang <strong>profesional dan berkualitas tinggi, cocok untuk iklan</strong>. Teknologi kami secara otomatis menyempurnakan perintah Anda untuk hasil yang halus dan berestetika teknologi tinggi.</p>
      <form onSubmit={handleBlendSubmit} className="space-y-4">
        <div>
          <label htmlFor="blend-image-upload" className="block text-sm font-medium text-gray-300 mb-2">Unggah Gambar (pilih beberapa)</label>
          <input
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold mb-4 text-indigo-400">Pembuatan & Pemaduan Gambar</h2>
      <div className="flex border-b border-gray-700 mb-6">
        <button 
          onClick={() => setActiveMode('generate')}
          className={`px-4 py-2 font-semibold transition-colors ${activeMode === 'generate' ? 'border-b-2 border-indigo-500 text-white' : 'text-gray-400 hover:text-white'}`}
        >
          Buat Gambar
        </button>
        <button 
          onClick={() => setActiveMode('blend')}
          className={`px-4 py-2 font-semibold transition-colors ${activeMode === 'blend' ? 'border-b-2 border-indigo-500 text-white' : 'text-gray-400 hover:text-white'}`}
        >
          Padukan Gambar
        </button>
      </div>
