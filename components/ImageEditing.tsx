...
const ControlWrapper: React.FC<{ title: string, children: React.ReactNode }> = ({ title, children }) => (
  <fieldset className="p-3 sm:p-4 border border-gray-700 rounded-lg space-y-3">
    <legend className="px-2 text-sm font-semibold text-indigo-300">{title}</legend>
    {children}
  </fieldset>
);
...
export const ImageEditing: React.FC<ImageEditingProps> = ({ onGenerateVideo, onCreationComplete, initialImage, onImageConsumed }) => {
  ...
  const [backgroundGradientStart, setBackgroundGradientStart] = useState('#374151');
  const [backgroundGradientEnd, setBackgroundGradientEnd] = useState('#111827');
...
`;
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold mb-4 text-indigo-400">Edit Gambar</h2>
      <p className="text-gray-400 mb-6">Buat komposisi profesional dengan menempatkan gambar Anda pada latar belakang khusus, menambahkan teks, dan logo.</p>

      <div className="mb-4">
        <label htmlFor="image-upload" className="block text-sm font-medium text-gray-300 mb-2">Unggah Gambar Produk (pilih beberapa)</label>
        <input
          id="image-upload"
          type="file"
          accept="image/*"
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            <div className="space-y-4">
              <h3 className="font-semibold text-lg text-gray-300">Panel Kontrol</h3>
              
              <ControlWrapper title="Gambar Produk yang Diunggah">
                <div className="flex flex-wrap gap-4 p-2 bg-gray-900/50 rounded-lg">
               ...
               <ControlWrapper title="Latar Belakang">
                <div>
                    <label className="text-xs text-gray-400 block mb-1">Jenis Latar Belakang</label>
                    <select value={backgroundType} onChange={(e) => setBackgroundType(e.target.value)} disabled={isLoading} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 text-sm">
                ...
                {backgroundType === 'solid' && (
                    <div>
                        <label className="text-xs text-gray-400 block mb-1">Warna Latar</label>
                        <input type="color" value={backgroundColor} onChange={(e) => setBackgroundColor(e.target.value)} disabled={isLoading} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/>
                    </div>
                )}
                {backgroundType === 'gradient' && (
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            <label className="text-xs text-gray-400 block mb-1">Awal Gradien</label>
                            <input type="color" value={backgroundGradientStart} onChange={(e) => setBackgroundGradientStart(e.target.value)} disabled={isLoading} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/>
                        </div>
                        <div>
                            <label className="text-xs text-gray-400 block mb-1">Akhir Gradien</label>
                            <input type="color" value={backgroundGradientEnd} onChange={(e) => setBackgroundGradientEnd(e.target.value)} disabled={isLoading} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/>
                        </div>
                    </div>
                )}
                {backgroundType === 'prompt' && (
                    <div>
                        <label className="text-xs text-gray-400 block mb-1">Deskripsi Latar</label>
                        <textarea
                            value={backgroundPrompt}
                            onChange={(e) => setBackgroundPrompt(e.target.value)}
                            placeholder="misalnya: sebuah meja kayu pedesaan dengan pencahayaan lembut"
                            className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 focus:ring-1 focus:ring-indigo-500 focus:outline-none resize-y"
                            rows={2}
                            disabled={isLoading}
                        />
                    </div>
              ...
              <ControlWrapper title="Pengaturan Output">
                <label className="text-xs text-gray-400 block mb-2">Rasio Aspek</label>
                ...
                <ControlWrapper title="Tambahkan Teks">
                    <div className="space-y-4">
                        {texts.map((textItem, index) => (
                            <div key={textItem.id} className="p-3 border border-gray-600 rounded-md bg-gray-800/50 relative space-y-3">
                                <div className="flex justify-between items-center">
                                    <h4 className="font-semibold text-gray-300">Teks #{index + 1}</h4>
                                    <button type="button" onClick={() => handleRemoveText(textItem.id)} className="text-red-400 hover:text-red-300 text-xs font-bold">HAPUS</button>
                                </div>
                                <input type="text" value={textItem.text} onChange={(e) => handleTextChange(textItem.id, 'text', e.target.value)} placeholder="Teks Anda di sini..." className="w-full p-2 bg-gray-700 rounded-md border border-gray-600"/>
                                <div className="flex items-center gap-4 text-sm text-gray-300">
                                    <div className="flex items-center"><input type="checkbox" id={`is3d-${textItem.id}`} checked={textItem.is3D} onChange={(e) => handleTextChange(textItem.id, 'is3D', e.target.checked)} className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-600 rounded focus:ring-indigo-500" /><label htmlFor={`is3d-${textItem.id}`} className="ml-2">3D</label></div>
                                    <div className="flex items-center"><input type="checkbox" id={`isGradient-${textItem.id}`} checked={textItem.isGradient} onChange={(e) => handleTextChange(textItem.id, 'isGradient', e.target.checked)} className="w-4 h-4 text-indigo-600 bg-gray-800 border-gray-600 rounded focus:ring-indigo-500" /><label htmlFor={`isGradient-${textItem.id}`} className="ml-2">Gradien</label></div>
                                </div>
                                {textItem.isGradient ? (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        <div><label className="text-xs text-gray-400 block">Awal</label><input type="color" value={textItem.gradientStart} onChange={(e) => handleTextChange(textItem.id, 'gradientStart', e.target.value)} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/></div>
                                        <div><label className="text-xs text-gray-400 block">Akhir</label><input type="color" value={textItem.gradientEnd} onChange={(e) => handleTextChange(textItem.id, 'gradientEnd', e.target.value)} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/></div>
                                    </div>
                                ) : (
                                    <div><label className="text-xs text-gray-400 block">Warna</label><input type="color" value={textItem.color} onChange={(e) => handleTextChange(textItem.id, 'color', e.target.value)} className="w-full h-9 p-1 bg-gray-700 rounded-md border border-gray-600 cursor-pointer"/></div>
                                )}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <div><label className="text-xs text-gray-400 block">Font</label><select value={textItem.font} onChange={(e) => handleTextChange(textItem.id, 'font', e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 text-sm">{fonts.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}</select></div>
                                    <div><label className="text-xs text-gray-400 block">Ukuran</label><select value={textItem.size} onChange={(e) => handleTextChange(textItem.id, 'size', e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 text-sm">{sizes.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <div><label className="text-xs text-gray-400 block">Posisi</label><select value={textItem.position} onChange={(e) => handleTextChange(textItem.id, 'position', e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 text-sm">{positions.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}</select></div>
                                    <div><label className="text-xs text-gray-400 block">Efek Teks</label><select value={textItem.effect} onChange={(e) => handleTextChange(textItem.id, 'effect', e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600 text-sm">{textEffects.map(e => <option key={e.value} value={e.value}>{e.label}</option>)}</select></div>
                                </div>
                            </div>
                        ))}
                        <button type="button" onClick={handleAddText} disabled={isLoading} className="w-full px-4 py-2 bg-indigo-500/20 text-indigo-300 font-semibold rounded-lg hover:bg-indigo-500/30 disabled:opacity-50 transition-colors">
                            + Tambah Teks Baru
                        </button>
                    </div>
                </ControlWrapper>
              ...
              <ControlWrapper title="Tambahkan Logo">
                <input type="file" accept="image/png, image/jpeg" onChange={handleLogoFileChange} className="block w-full text-sm text-gray-400 file:mr-4 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-600 file:text-gray-200 hover:file:bg-gray-500"/>
                {logoImage && <img src={logoImage.url} alt="Pratinjau logo" className="h-16 w-auto bg-white/10 p-1 rounded-md" />}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                   <div>
                      <label className="text-xs text-gray-400 block">Ukuran</label>
                      <select value={logoSize} onChange={(e) => setLogoSize(e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600">
                        {sizes.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="text-xs text-gray-400 block">Posisi</label>
                      <select value={logoPosition} onChange={(e) => setLogoPosition(e.target.value)} className="w-full p-2 bg-gray-700 rounded-md border border-gray-600">
                        {positions.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
                      </select>
                    </div>
                </div>
                 <div>
                    <label className="text-xs text-gray-400 block">Opasitas: {logoOpacity}%</label>
                    <input type="range" min="0" max="100" value={logoOpacity} onChange={(e) => setLogoOpacity(Number(e.target.value))} className="w-full h-2 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-indigo-500"/>
                  </div>
              </ControlWrapper>
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-lg text-gray-300">Hasil</h3>
              <div className="bg-gray-900/50 rounded-lg border border-gray-700 min-h-[16rem] md:min-h-[24rem] h-full flex justify-center items-center p-4">
