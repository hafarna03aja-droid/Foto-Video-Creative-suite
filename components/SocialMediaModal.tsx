...
export const SocialMediaModal: React.FC<SocialMediaModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4" onClick={onClose}>
      <div
        className="bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col md:flex-row gap-6 p-6 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Left side: Media Preview */}
        <div className="w-full md:w-1/3 flex-shrink-0">
          <h3 className="text-lg font-bold text-indigo-300 mb-4">Pratinjau Media</h3>
          <div className="rounded-lg overflow-hidden aspect-square bg-gray-900 flex items-center justify-center">
            {mediaType === 'image' ? (
        ...
        <div className="w-full md:w-2/3 flex flex-col overflow-y-auto">
          <div className="flex justify-between items-start mb-4">
             <div>
                <h2 className="text-xl font-bold text-indigo-300">Teks Promosi yang Dihasilkan</h2>
                <p className="text-sm text-gray-400">Salin teks untuk platform pilihan Anda.</p>
             </div>
             <button
                onClick={onClose}
          ...
          <div className="space-y-4 pr-2">
            ...
            {!isLoading && !error && posts.map((post) => (
              <div key={post.platform} className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-bold text-lg text-gray-200">{post.platform}</h4>
                  <button
                    onClick={() => handleCopy(post.platform, post.content)}
                    className="flex items-center gap-2 text-sm px-3 py-1 bg-gray-700 hover:bg-gray-600 rounded-md transition-colors"
                  >
                    <CopyIcon />
