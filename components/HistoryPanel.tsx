...
const HistoryItemCard: React.FC<{ item: HistoryItem; onReusePrompt: (item: HistoryItem) => void; onDownload: (item: HistoryItem) => void; }> = ({ item, onReusePrompt, onDownload }) => {
  ...
  const renderPreview = () => {
    ...
    return <div className="text-gray-500"><TypeIcon type={item.type} /></div>;
  };

  return (
    <div className="bg-gray-800/50 p-3 rounded-lg flex gap-3">
      <div className="w-24 h-24 flex-shrink-0 bg-gray-900 rounded-md flex items-center justify-center overflow-hidden">
        {renderPreview()}
      </div>
      <div className="flex-grow overflow-hidden">
        ...
        <p className="text-xs text-gray-400 mt-1 truncate" title={item.prompt}>{item.prompt}</p>
        <div className="flex gap-2 mt-2">
          <button onClick={() => onReusePrompt(item)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md">Gunakan Ulang</button>
          <button onClick={() => onDownload(item)} className="px-2 py-1 text-xs bg-green-700 hover:bg-green-600 rounded-md flex items-center gap-1"><DownloadIcon /> Unduh</button>
        </div>
      </div>
    </div>
  );
...
export const HistoryPanel: React.FC<HistoryPanelProps> = ({ isOpen, onClose, items, onClear, onReusePrompt }) => {
  ...
  return (
    <>
      ...
      ></div>
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-gray-900 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        } flex flex-col`}
      >
        <div className="p-4 border-b border-gray-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-indigo-300">Riwayat Kreasi</h2>
        ...
        {items.length > 0 && (
          <div className="p-4 border-t border-gray-700">
            <button 
              onClick={onClear} 
              className="w-full py-2 px-4 bg-red-800/50 text-red-300 hover:bg-red-800/80 rounded-lg transition-colors"
            >
              Hapus Semua
            </button>
          </div>
