...
export const VideoGeneration: React.FC<VideoGenerationProps> = ({ initialImage, initialPrompt, onLeave, onCreationComplete }) => {
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl font-bold mb-4 text-indigo-400">Pembuatan Video (Veo)</h2>
      <p className="text-gray-400 mb-6">Buat video definisi tinggi dari teks atau gambar awal. Proses pembuatan mungkin memakan waktu beberapa menit.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
            <label htmlFor="video-upload" className="text-sm font-medium text-gray-300">Gambar awal (opsional):</label>
            <input
              id="video-upload"
              type="file"
              accept="image/*"
              onChange={handleFileChange}
              className="text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-gray-600 file:text-gray-200 hover:file:bg-gray-500"
              disabled={isLoading}
            />
        </div>
      ...
      <div className="mt-6">
        {isLoading && (
          <div className="flex flex-col justify-center items-center h-64 bg-gray-700/50 rounded-lg">
            <Spinner />
            <span className="ml-2 mt-4 font-semibold">{loadingMessage}</span>
            <span className="text-sm text-gray-400 mt-2">Pembuatan video dapat memakan waktu beberapa menit. Mohon bersabar.</span>
          </div>
        )}
        {videoUrl && (
