...
export const PromptBuilder: React.FC<PromptBuilderProps> = ({ onUsePrompt }) => {
    ...
    return (
        <div className="flex flex-col h-full">
            <h2 className="text-2xl font-bold mb-4 text-indigo-400">Asisten Prompt</h2>
            <p className="text-gray-400 mb-6">Ubah ide sederhana Anda menjadi prompt yang kaya detail untuk hasil gambar dan video yang menakjubkan. Jelaskan ide Anda, dan biarkan AI menyempurnakannya.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <textarea
                    ...
                    className="w-full p-3 bg-gray-700 rounded-lg border border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:outline-none resize-none"
                    rows={4}
                    disabled={isLoading}
                />
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                disabled={isLoading}
                                className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors flex items-center gap-2 ${target === 'image' ? 'bg-indigo-600 text-white' : 'bg-gray-700 hover:bg-gray-600'}`}
                            >
                                <ImageIcon /> Gambar
                            </button>
                            <button
                                ...
                                disabled={isLoading}
                                className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors flex items-center gap-2 ${target === 'video' ? 'bg-indigo-600 text-white' : 'bg-gray-700 hover:bg-gray-600'}`}
                            >
                                <VideoIcon /> Video
                            </button>
                        </div>
            ...
            {(isLoading || enhancedPrompt) && (
                <div className="mt-6 flex-grow flex flex-col">
                    <h3 className="font-semibold text-lg mb-2 text-indigo-300">Prompt yang Disempurnakan:</h3>
                    <div className="flex-grow p-4 bg-gray-900/50 rounded-lg border border-gray-700 min-h-[150px]">
                        {isLoading 
                            ? <div className="flex items-center text-gray-400"><Spinner /> Menciptakan prompt yang sempurna...</div>
